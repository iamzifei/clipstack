...
- 菜单栏常驻（无 Dock 图标），每 0.25s 监控系统剪贴板
...
- 菜单栏下拉：最近 10 条一键复制、暂停监控、清空历史（保留置顶）、开机自启
...
无偏好界面，用 `defaults` 配置（重启 App 生效）：

```bash
# 改全局快捷键（Carbon keycode + modifier；默认 9=V, 768=⇧⌘）
defaults write com.james.ClipStack HotKeyKeyCode -int 9
defaults write com.james.ClipStack HotKeyModifiers -int 768
# 常用 modifiers：⌘=256 ⇧=512 ⌥=2048 ⌃=4096（相加组合）
