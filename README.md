...
- 全局快捷键 **⇧⌘V** 呼出切换面板：搜索（支持中文输入法）、↑/↓ 选择、**回车 = 复制并关闭**、⌘1–9 快速复制、⌘P 置顶该条、**⌘T 窗口置顶**、⌘⌫ 删除、esc 关闭；右侧完整预览（文本等宽显示 / 图片缩放 / 文件路径列表）
- **⌘P 置顶的条目排在列表最前面**（面板与菜单栏下拉都是），不用翻找；再按一次 ⌘P 取消，条目回到按时间排的位置
- **⌘T 窗口置顶**（footer 高亮显示当前状态，重启后保留）：面板浮在所有窗口之上，失焦不关闭、复制后也不关闭——可以当成对照用的便签，一边看一边在别的窗口里输入粘贴。再按 ⌘T 关闭该模式，面板恢复「点别处即关闭」的行为
- 复制成功后屏幕顶部弹出「已复制」Toast 反馈（含内容摘要，1.3s 自动消失，不抢焦点）
- 全局快捷键 **⌘,** 打开设置面板：开机自启动开关、三个全局快捷键点击录制改键（冲突检测、Esc 取消），存储于 UserDefaults
...
defaults write com.james.ClipStack SettingsHotKeyKeyCode -int 43   # 设置，默认 ⌘,
defaults write com.james.ClipStack SettingsHotKeyModifiers -int 256
defaults write com.james.ClipStack ClearAllHotKeyKeyCode -int 51   # 清空所有，默认 ⇧⌘⌫
defaults write com.james.ClipStack ClearAllHotKeyModifiers -int 768
```
